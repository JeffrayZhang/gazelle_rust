...
func parseSources(srcs build.Expr, c *config.Config, from label.Label) []string {
	if srcs == nil {
		return []string{}
	}
	items := srcs.(*build.ListExpr).List
	fileNames := []string{}
	for _, item := range items {
		file := item.(*build.StringExpr).Value
		if strings.HasSuffix(file, ".rs") {
			fullPath := path.Join(c.RepoRoot, from.Pkg, file)
...
func (e *Extension) Resolve(c *config.Config, ix *resolve.RuleIndex, rc *repo.RemoteCache, r *rule.Rule, imports interface{}, from label.Label) {
	...
	srcs := r.Attr("srcs")
	fmt.Printf("srcs: %#v\n", srcs)
	fileContents := parseSources(srcs, c, from)
	for _, fileName := range fileContents {
		for _, dep := range ParseFile(fileName) {
			deps.Add(dep)
		}
	}
	fmt.Printf("setting deps: %#v\n", deps.ToSlice())
	r.SetAttr("deps", deps.ToSlice())
}
